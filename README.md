...
2. Prompt for your license key and GCP project ID
3. Auto-generate all secrets (database password, JWT, encryption key)
4. Pull and start PostgreSQL + QueryFuser

A default BigQuery project is created automatically when you register your first user. The proxy uses the VM's default service account — no service account JSON needed.
...
| `BIGQUERY_DATASET` | | Default BigQuery dataset (can be changed in the dashboard) |
...
By default, QueryFuser runs a PostgreSQL 16 container alongside the proxy. Data is persisted in a Docker volume called `pgdata`.

### Backups

```bash
# Dump the database
docker compose exec db pg_dump -U postgres queryfuser > backup.sql

# Restore from a backup
cat backup.sql | docker compose exec -T db psql -U postgres queryfuser
```
...
4. **Update `.env`** — add a `DATABASE_URL` pointing to Cloud SQL:
   ```
   DATABASE_URL=postgres://postgres:PASSWORD@CLOUD_SQL_IP:5432/queryfuser
   ```

5. **Restart QueryFuser:**
   ```bash
   docker compose up -d
   ```

   When `DATABASE_URL` is set, the bundled `db` container still starts but is unused. You can optionally stop it:
   ```bash
